 ...
 * Frames are nearest-neighbor fitted into `target` (default 128×128).
 * Pass the artboard size to use the GIF as the whole sheet.
 */
export function gifToSprite(
  ...
  if (n > GIF_MAX_FRAMES) throw new GifError(`too many frames (${n} > ${GIF_MAX_FRAMES})`);
  ...
  let lastW = 0;
  let lastH = 0;
  const strip = new Uint8Array(tw * n * th * 4);
  let delaySum = 0;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const fitted = fitRgba(composed, fw, fh, tw, th);
    blitFrame(strip, tw * n, tw, th, i * tw, fitted);
  ...
  const png = encodePngRgba(tw * n, th, strip);
  const avgCs = delaySum / n;
  const fps = Math.max(1, Math.round(100 / avgCs));
  const dur = `${(n / fps).toFixed(2)}s`;
...
/** Contain-fit RGBA into dw×dh with transparent padding. Nearest-neighbor for pixel art. */
